...
export default async function OpengraphImage() {
  const imgData = await readFile(join(process.cwd(), 'public/images/self portrait.jpg'));
  ...
  return new ImageResponse(
    (
      <div style={{ width: '100%', height: '100%', display: 'flex', position: 'relative', overflow: 'hidden' }}>
        <img src={imgBase64} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top' }} />
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '60px 80px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 14, letterSpacing: 6, textTransform: 'uppercase', color: '#c9a84c', marginBottom: 20, fontFamily: 'sans-serif' }}>About</div>
            <div style={{ fontSize: 72, lineHeight: 1.05, color: '#eae6df', fontFamily: 'serif', display: 'flex', flexDirection: 'column' }}>
              <span>Matthew</span>
              <span style={{ color: '#c9a84c', fontStyle: 'italic' }}>Strohm</span>
            </div>
            <div style={{ fontSize: 18, color: '#8a857e', marginTop: 20, fontFamily: 'sans-serif', maxWidth: 520 }}>
              Photographer, videographer &amp; creative based in Aberdeenshire
            </div>
          </div>
